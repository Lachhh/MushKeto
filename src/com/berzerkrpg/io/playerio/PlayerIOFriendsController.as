package com.berzerkrpg.io.playerio {
	...
	import com.berzerkrpg.io.MetaFriend;
	...
	public class PlayerIOFriendsController {
		...
		public function loadGame(metaFriend:MetaFriend, success:Callback, error:Callback):void {
			PlayerIOZombIdleController.getInstance().myPublicConnection.client.bigDB.load(
				"PlayerObjects", 
				...
				function (user:DatabaseObject):void{
					if(user != null) {
						...
						metaFriend.setMetaPlayerFromRawData(user);
		...
		public function AddApproveFriendRequest(metaFriend:MetaFriend, success:Callback, error:Callback):void {
			if(!PlayerIOZombIdleController.getInstance().mySecuredConnection.connected) return ;
			...
			PlayerIOZombIdleController.getInstance().mySecuredConnection.client.bigDB.load(
				"FriendsVisited", 
				GetMyAccountName(), 
				function (user:DatabaseObject):void{
					if(user != null) {
